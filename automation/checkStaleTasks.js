...
async function main() {
  ...
  for (const row of users) {
    ...
    for (const t of rows) {
      ...
      if (daysSince >= STALE_DAYS_THRESHOLD) {
        staleTasks.push({ fullName: fullName, taskText: taskText, status: taskStatus, daysSince: daysSince });
      }
    }
  }
  ...
  fs.writeFileSync(path.join(alertsDir, 'summary.json'), JSON.stringify({ count: staleTasks.length, generatedAt: new Date().toISOString() }));
